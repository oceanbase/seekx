...
describe("SeekxClient construction", () => {
    test("can be instantiated without throwing", () => {
        const client = new SeekxClient({ baseUrl: "http://localhost:11434/v1", apiKey: "", model: "nomic-embed-text" }, null, null);
        expect(client).toBeDefined();
    });
});
describe("SeekxClient.embed — unreachable endpoint", () => {
    test("returns null (fail-open) on network error", async () => {
        const client = new SeekxClient({ baseUrl: "http://127.0.0.1:1", apiKey: "", model: "test" }, null, null);
